...
def candidate_duplicate_of(candidate: AtlasUpdateCandidate, index: dict[str, str]) -> str | None:
    keys = [
        f"title:{normalize_title(candidate.title)}",
        f"doi:{candidate.doi.lower()}",
        f"arxiv:{candidate.arxiv_id.lower()}",
...
def parse_json_candidates(raw_text: str) -> list[dict[str, Any]]:
...
def coerce_candidate(raw: Any, atlas_id: str, run_id: str, route_ids: set[str]) -> AtlasUpdateCandidate | None:
    ...
    if route_id and route_id not in route_ids:
        # Keep AI's suggestion visible, but avoid breaking layout by falling back later.
        route_id = route_id
    ...
    return AtlasUpdateCandidate(
        id=safe_id_or_slug(raw.get("id"), "cand"),
        status="pending",
        title=title,
        authors=authors,
        year=raw.get("year"),
        venue=safe_run_text(raw.get("venue"), 160),
        url=safe_run_text(raw.get("url") or raw.get("paper_url"), 400),
        doi=safe_run_text(raw.get("doi"), 160),
        arxiv_id=safe_run_text(raw.get("arxiv_id") or raw.get("arxiv"), 120),
        abstract=str(raw.get("abstract") or raw.get("summary") or "")[:2400],
        ...
        tags=[str(item).strip() for item in (raw.get("tags") or []) if str(item).strip()] if isinstance(raw.get("tags"), list) else [],
        created_at=now,
        updated_at=now,
    )


def extract_update_candidates(raw_text: str, atlas_id: str, run_id: str, route_ids: set[str]) -> list[AtlasUpdateCandidate]:
    parsed_objects = parse_json_candidates(raw_text)
    ...
    for item in raw_candidates:
        candidate = coerce_candidate(item, atlas_id, run_id, route_ids)
        if candidate:
            result.append(candidate)
    return result


def atlas_update_action_title(action: str) -> str:
    return {
        "recent": "更新最新论文",
        "all": "更新全部论文",
...
def build_atlas_update_task_pack(atlas_id: str, payload: AtlasUpdateTaskPackRequest) -> AtlasUpdateTaskPackResponse:
    bundle = atlas_bundle_for_update(atlas_id)
    updates = load_atlas_updates(atlas_id)
    action_title = atlas_update_action_title(payload.action)
    routes = bundle.get("routes", []) or []
    papers = bundle.get("papers", []) or []
    relations = bundle.get("relations", []) or []
    route_filter = payload.route_id
    route_ids = {str(route.get("id")) for route in routes if route.get("id")}
    route_lines = []
    for route in routes:
        rid = str(route.get("id") or "")
        if route_filter and rid != route_filter:
            continue
        route_papers = [p for p in papers if str(p.get("route_id") or p.get("raw_fields", {}).get("routeName") or "") == rid]
        route_lines.append(
            f"- {rid}: {route.get('name') or route.get('cn') or rid} | {len(route_papers)} 篇 | {route.get('rationale') or ''}"
        )
    ...
    pending_count = len([c for c in updates.candidates if c.status == "pending"])
    instruction = payload.instruction_override or ""
    if not instruction:
        if payload.action == "recent":
            instruction = "重点查找最近 1-2 年与本 Atlas 强相关、尚未出现的新论文。"
        elif payload.action == "all":
            instruction = "从整体覆盖角度查找应收录但缺失的重要论文，不限年份。"
        elif payload.action == "complete_cards":
            instruction = "审查已有论文卡，指出需要补全摘要、核心贡献、局限或路线归类的对象。"
        else:
            instruction = "补充项目页、代码、后续工作、复现实验或关键证据来源。"
    ...
    markdown = "\n".join(
        [
            ...
            f"- pending_personal_candidates: {pending_count}",
            "",
            "## 路线结构",
            "\n".join(route_lines) or "(暂无路线)",
            "",
            "## 年份分布",
            ", ".join(f"{year}:{count}" for year, count in sorted(year_counts.items(), reverse=True)[:24]),
            "",
            "## 已有论文查重清单",
...
def apply_candidate_to_personal_memory(atlas_id: str, candidate: AtlasUpdateCandidate) -> None:
    object_id = candidate.id or slug_id("cand")
    memory = effective_object_memory(atlas_id, "paper", object_id) or ObjectMemory(
    ...
    if candidate.judgement:
        memory.judgement = candidate.judgement
    elif not memory.judgement:
        memory.judgement = candidate.why or candidate.relevance
    if not memory.note:
        memory.note = "\n".join(
            part for part in [
